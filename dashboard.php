    ...
    try {
        switch ($action) {
            ...
            case 'save_signature':
                ...
                if (!preg_match('/^data:image\/png;base64,/', $signature) || strlen($signature) > 3 * 1024 * 1024) {
                    throw new RuntimeException('Dibuja una firma válida antes de guardar.');
                }
                $stmt = $pdo->prepare('INSERT INTO agenda_firmas (usuario_id, nombre, imagen) VALUES (?, ?, ?)');
                $stmt->execute([$userId, input('firma_nombre', 'Firma principal'), $signature]);
                audit($userId, 'Creó una firma digital dibujada');
                flash('success', 'Firma guardada. Puedes usarla como referencia en tus documentos.');
                redirect('dashboard.php#firma');
                break;

            case 'delete_signature':
                if (($_POST['confirm_delete'] ?? '0') !== '1') {
                    throw new RuntimeException('Confirma la eliminación de la firma.');
                }
                $pdo->prepare('DELETE FROM agenda_firmas WHERE id = ? AND usuario_id = ?')->execute([(int) ($_POST['signature_id'] ?? 0), $userId]);
...
$signatureStmt = $pdo->prepare('SELECT * FROM agenda_firmas WHERE usuario_id = ? ORDER BY creado_en DESC LIMIT 8');
$signatureStmt->execute([$userId]);
...
<div class="app-layout">
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="main-nav" aria-label="Navegación principal">
            ...
            <a href="#firma" class="nav-link"><span>✎</span> Firma digital</a>
    ...
    <div class="page-content">
        <header class="topbar">
            ...
            <div class="topbar-actions"><button class="icon-button" id="notificationButton" title="Recordatorios">♢</button><span class="topbar-date"><?= e($todayLabel) ?></span><a class="btn btn-primary btn-compact" href="#agenda">+ Nuevo evento</a></div>
        </header>
        ...
        <main class="dashboard-main" id="resumen">
            ...
            <section class="content-grid lower-grid"><article class="panel" id="firma"><div class="panel-heading"><div><p class="eyebrow">Identidad visual</p><h2>Firma digital</h2></div><span class="secure-chip">Privada</span></div><p class="panel-description">Dibuja una firma de referencia para añadirla a tus documentos.</p><div class="signature-box"><canvas id="signatureCanvas" width="560" height="180"></canvas><span id="signaturePlaceholder">Dibuja aquí con el mouse o tu dedo</span></div><div class="signature-actions"><form method="post" id="signatureForm"><input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="save_signature"><input type="hidden" name="firma_nombre" value="Firma principal"><input type="hidden" name="signature_data" id="signatureData"><button class="btn btn-primary" type="submit">Guardar firma</button></form><button class="btn btn-ghost" type="button" id="clearSignature">Limpiar</button></div><?php if ($signatures): ?><div class="saved-signatures"><?php foreach ($signatures as $signature): ?><div><img src="<?= e($signature['imagen']) ?>" alt="Firma guardada"><form method="post" class="inline-form" onsubmit="return confirmDelete(this, 'esta firma')"><input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="delete_signature"><input type="hidden" name="signature_id" value="<?= (int) $signature['id'] ?>"><input type="hidden" name="confirm_delete" value="0"><button class="row-action danger" type="submit">×</button></form></div><?php endforeach; ?></div><?php endif; ?></article>
                <article class="panel" id="claves"><div class="panel-heading"><div><p class="eyebrow">Protección AES-256</p><h2>Bóveda de claves</h2></div><span class="secure-chip">Cifrada</span></div><p class="panel-description">Guarda credenciales sin almacenarlas en texto plano. La contraseña de acceso nunca se muestra por defecto.</p><form method="post" class="form-grid compact-form"><input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="save_key"><label>Servicio<input name="servicio" placeholder="Correo, banco, portal..." required></label><label>Usuario <span class="optional">opcional</span><input name="usuario_clave" placeholder="usuario o correo"></label><label class="full">Clave<input type="password" name="clave" placeholder="Se cifra antes de guardar" required></label><div class="form-actions full"><button class="btn btn-secondary" type="submit">Guardar clave</button></div></form><div class="key-list"><?php foreach ($keys as $key): ?><div class="key-row"><span class="key-symbol">▣</span><div class="row-main"><strong><?= e($key['servicio']) ?></strong><span><?= e($key['usuario'] ?: 'Sin usuario') ?></span></div><form method="post" class="inline-form"><input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="show_key"><input type="hidden" name="key_id" value="<?= (int) $key['id'] ?>"><button class="row-action" type="submit">Ver</button></form><form method="post" class="inline-form" onsubmit="return confirmDelete(this, 'esta clave')"><input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="delete_key"><input type="hidden" name="key_id" value="<?= (int) $key['id'] ?>"><input type="hidden" name="confirm_delete" value="0"><button class="row-action danger" type="submit">×</button></form></div><?php endforeach; ?><?php if (!$keys): ?><div class="empty-inline">No hay claves guardadas todavía.</div><?php endif; ?></div><?php if ($revealedSecret): ?><div class="secret-reveal"><strong><?= e($revealedSecret['servicio']) ?></strong><code><?= e($revealedSecret['value']) ?></code><span>Cierra esta pestaña o elimina este mensaje cuando termines.</span></div><?php endif; ?></article></section>
